...
class Image
{
   ...
   function create2($set,$stageWidth,$stageHeight)
   {
      ...
      ob_end_clean();
         ...
         // write to file
      $this->filename = $this->path."tri_". rand(0,1000).".png";
   ...
   function create()
   {
      ...
      foreach ($this->delaunay as $key => $arr)
      {
		 foreach ($arr as $ikey => $iarr)
		 {
			...
			if (abs($x1) != SUPER_TRIANGLE && abs($y1) != SUPER_TRIANGLE && abs($x2) != SUPER_TRIANGLE && abs($y2) != SUPER_TRIANGLE)
			{
			  ...
			  imagefilledellipse($im, $x1+$this->padding, $y1+$this->padding, 10, 10, $blue);
			}
		}
    }
      ...
      foreach ($this->geofence as $key => $arr)
      {
		 foreach ($arr as $ikey => $iarr)
		 {
			...
			if (abs($x1)!= SUPER_TRIANGLE && abs($y1)!= SUPER_TRIANGLE && abs($x2) != SUPER_TRIANGLE && abs($y2) != SUPER_TRIANGLE)
			{
			   imageline($im,$x1+$this->padding,$y1+$this->padding,$x2+$this->padding,$y2+$this->padding,$firebrick); 	       
			}
		 }
      }
